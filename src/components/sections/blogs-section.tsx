  ...
  return (
    <section className="bg-background py-32 px-6 md:px-12 relative overflow-hidden border-t border-white/5">
      <div className="container mx-auto relative z-10">
        ...
        {/* Horizontal blogs carousel — faster, user can scroll/drag */}
        <div className="relative max-w-6xl mx-auto">
          ...
          <ScrollableMarquee speed={130} className="py-4" innerClassName="flex gap-8">
            {safeBlogs.map((blog, idx) => (
              <motion.article
                key={`${blog.id}-${idx}`}
              ...
              >
                <div className="flex items-center gap-3 text-xs text-muted-foreground mb-4">
                  ...
                  <div className="flex flex-col">
                    <span className="font-semibold text-white/80 text-[11px] uppercase tracking-[0.18em]">{blog.author}</span>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed mb-4 line-clamp-3">{blog.excerpt}</p>

                <Link href={`/blogs/${blog.id}`} className="mt-auto inline-flex items-center text-[11px] font-bold uppercase tracking-[0.25em] text-primary hover:text-primary/80">
                  ...continue reading
                </Link>
              </motion.article>
            ))}
