      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-16 md:mb-32">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-4 px-4 py-1.5 md:px-6 md:py-2 rounded-full bg-white/5 border border-white/10 text-primary text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] mb-8 md:mb-12 shadow-2xl ring-1 ring-white/5"
          >
            <Building2 className="w-4 h-4" />
            <span>Enterprise Innovation Grid</span>
          </motion.div>
          <motion.h2 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-4xl sm:text-6xl md:text-8xl font-black text-white mb-6 md:mb-10 tracking-tighter uppercase leading-[0.9]"
          >
            Global <span className="text-gradient-brand">Innovation</span> Centers
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-muted-foreground text-lg md:text-2xl max-w-3xl mx-auto font-medium leading-relaxed tracking-tight opacity-60"
          >
            Strategically architected hubs across global tech epicenters facilitating rapid enterprise scale and disruption.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {hubs.map((hub, index) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="group relative flex flex-col bg-slate-950/40 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.6)] transition-all duration-700 hover:scale-[1.02] hover:border-primary/40 h-full ring-1 ring-white/5"
            >
              ...
              <div className="p-10 flex flex-col flex-1">
                <div className="flex items-center gap-4 mb-8 text-muted-foreground">
                   ...
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black text-white/40 tracking-[0.2em] uppercase">Enterprise Hub</span>
                      <span className="text-[11px] font-bold text-white tracking-widest uppercase">{hub.location}</span>
                   </div>
                </div>

                <h3 className="text-3xl font-black text-white leading-[1.1] tracking-tighter uppercase mb-10 group-hover:text-primary transition-colors">
                  {hub.collaboration}
                </h3>

                <div className="mt-auto space-y-6">
                  <div className="flex items-center justify-between py-4 border-y border-white/5">
                     <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Protocol Capacity</span>
                     <span className="text-[12px] font-bold text-white uppercase">{hub.stats}</span>
                  </div>

                  <button 
                    disabled={hub.isComingSoon}
                    className={`group/btn w-full py-5 text-[10px] font-black uppercase tracking-[0.4em] rounded-2xl transition-all duration-300 transform active:scale-95 shadow-2xl flex items-center justify-center gap-3 ${
                      hub.isComingSoon 
                  ...
                  >
                    ...
                    {!hub.isComingSoon && <ArrowUpRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1" />}
                  </button>
                </div>
              </div>
            </motion.div>
